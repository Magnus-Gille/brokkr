...
export function createBoundedRecoveryDispatcher({ recovery, attemptId, descriptorDigest, authorizeBinding, publishActivation, dispatch }) {
  if (!plain(recovery) || !ID.test(attemptId) || !DIGEST.test(descriptorDigest) || typeof authorizeBinding !== "function" || typeof publishActivation !== "function" || typeof dispatch !== "function") fail("bounded_recovery_dispatch_contract_invalid");
  return {
    ...recovery,
    recover: request => {
      ...
      if (!ID.test(request.idempotency_key)) fail("bounded_recovery_dispatch_id_invalid");
      if (request.descriptor_digest !== descriptorDigest || authorizeBinding({ binding_digest: request.binding_digest, descriptor_digest: request.descriptor_digest, target_scope_digest: request.target_scope_digest }) !== true) fail("bounded_recovery_dispatch_binding_invalid");
      if (!DIGEST.test(request.target_scope_digest) || !DIGEST.test(request.lease_fence_digest) || !DIGEST.test(request.revalidation_fence_digest)) fail("bounded_recovery_dispatch_digest_invalid");
      ...
      const activation = {
        ...
        binding_digest: request.binding_digest, recovery_descriptor_digest: descriptorDigest,
