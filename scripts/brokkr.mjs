...
const LOCATION_TOKEN = /^locv1_[A-Za-z0-9_-]{43,86}$/;
...
if (!closed(node, ["location", "remote_dir", "ssh_target", "signing_public_key"]) ||
  ...
  Date.parse(node.location.observed_at) > Date.parse(now) || Date.parse(node.location.valid_until) <= Date.parse(node.location.observed_at) ||
  !["known", "partial", "unknown"].includes(node.location.provenance) || typeof node.location.location_secret !== "string" ||
  !LOCATION_TOKEN.test(node.location.location_secret)) fail("inspection overlay has invalid node configuration");
...
const locationStale = Date.parse(node.location.valid_until) < Date.parse(now);
...
if (child.error || child.status !== 0) { output("unreachable", { freshness: locationStale ? "stale" : "current" }); process.exit(2); }
let capability;
try { capability = JSON.parse(child.stdout); } catch { output("partial", { freshness: locationStale ? "stale" : "current", reason: "invalid-agent-record" }); process.exit(2); }
if (capability.node_id !== nodeId) { output("identity-mismatch", { freshness: locationStale ? "stale" : "current" }); process.exit(2); }
if (schemaErrors(schema, capability).length || capability.evidence.digest !== `sha256:${evidenceDigest(capability)}` || capability.evidence.evidence_id !== observationEvidenceId(capability)) { output("partial", { freshness: locationStale ? "stale" : "current", reason: "invalid-shared-contract" }); process.exit(2); }
const detailLine = child.stderr.split("\n").find((line) => line.startsWith("Brokkr node inventory detail JSON: "));
let detail;
try { detail = JSON.parse(detailLine.slice("Brokkr node inventory detail JSON: ".length)); } catch { output("partial", { freshness: locationStale ? "stale" : "current", reason: "missing-signed-detail" }); process.exit(2); }
...
if (!closed(detail, ["backup_roles", "detail_digest", "kind", "observation_digest", "observation_evidence_id", "observed_at", "schema_version", "signature", "signing_key_id", "unit_state", "valid_until", "workloads"]) ||
  ...
  detail.observed_at !== capability.observed_at || detail.valid_until !== capability.valid_until) { output("partial", { freshness: locationStale ? "stale" : "current", reason: "unbound-signed-detail" }); process.exit(2); }
...
if (detail.detail_digest !== expectedDigest) { output("partial", { freshness: locationStale ? "stale" : "current", reason: "invalid-detail-digest" }); process.exit(2); }
try {
...
} catch { output("partial", { freshness: locationStale ? "stale" : "current", reason: "untrusted-signed-detail" }); process.exit(2); }
const capabilityStale = Date.parse(capability.valid_until) < Date.parse(now);
const freshness = locationStale || capabilityStale ? "stale" : "current";
output(freshness === "stale" || capability.capability_status !== "known" || node.location.provenance !== "known" ? "partial" : "ok", {
  freshness,
