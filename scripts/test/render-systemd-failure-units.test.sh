...
check "renderer rewrites every runtime service user" 'grep -Fqx "User=operator" "$OUT_DIR/brokkr-health.service" && grep -Fqx "User=operator" "$OUT_DIR/brokkr-maintenance-os.service" && grep -Fqx "User=operator" "$OUT_DIR/brokkr-maintenance-deps.service" && grep -Fqx "User=operator" "$OUT_DIR/brokkr-systemd-failure@.service" && grep -Fqx "User=operator" "$OUT_DIR/brokkr-systemd-failure-sweep.service"'
check "renderer rewrites health runtime home and deploy target" 'grep -Fqx "WorkingDirectory=/srv/brokkr" "$OUT_DIR/brokkr-health.service" && grep -Fqx "EnvironmentFile=-/home/operator/.config/brokkr/env" "$OUT_DIR/brokkr-health.service" && grep -Fqx "ExecStart=/srv/brokkr/scripts/health-snapshot.sh" "$OUT_DIR/brokkr-health.service"'
