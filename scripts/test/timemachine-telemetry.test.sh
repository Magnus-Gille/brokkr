...
#!/usr/bin/env bash
if [ "${MOCK_PYTHON_FAIL:-}" = snapshot ] && [ "$1" = - ] && [ "$#" -eq 4 ]; then exit 70; fi
if [ "${MOCK_PYTHON_FAIL:-}" = probe ] && [[ "$1" == *destination-probe.py ]]; then
  printf '%s\n' '{"status":"warn","reason":"mock_execution_failure","observed_at":1783947600,"count":0,"size_bytes":0}'
  exit 70
fi
...
run_delivery
check "probe status/exit mismatch fails before delivery" '[[ "$RC" -eq 2 && "$OUT" == *"cannot generate snapshot"* && ! -e "$TMP/request.json" ]]'
check "probe status/exit mismatch does not replace the old snapshot" '[[ "$(cat "$TMP/state/timemachine-health.json")" == "$previous_snapshot" ]]'
