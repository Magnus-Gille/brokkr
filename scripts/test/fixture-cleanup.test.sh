...
HERE="$1"; target="$2"; requested_status="$3"
readonly target
# shellcheck source=/dev/null
source "$HERE/lib/fixture-cleanup.sh"
trap 'fixture_cleanup_on_exit "$target"' EXIT
...
run_case() {
  ...
  MOCK_RM_MODE="$mode" MOCK_RM_STATE="$state" MOCK_RM_CALLS="$calls" \
    MOCK_RM_TARGET="$fixture" PATH="$TMP:$PATH" \
...
check "transient cleanup retries and removes fixture" '[[ "$(cat "$TMP/state-transient-17")" -eq 2 && ! -e "$fixture" ]]'
...
check "persistent cleanup passes only the fixture target" '[[ "$(grep -Fc -- "-rf -- $fixture" "$TMP/calls-persistent-23")" -eq 3 ]]'
check "persistent cleanup leaves unrelated data intact" '[[ -f "$TMP/unrelated/marker" && -d "$fixture" ]]'
