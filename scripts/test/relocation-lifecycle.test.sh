...
const plan={kind:'brokkr-relocation-plan',schema_version:'v1',plan_id:'plan-001',plan_digest:life.plan_digest,outcome:'promoted',lifecycle_result:life,rollback:{available:true,hook:'rollback'},hooks:[{name:'preflight',idempotency_required:true},{name:'drain',idempotency_required:true},{name:'verify',idempotency_required:true}]};
const hooks=['preflight','drain','apply','verify','representative_data','rollback'].map(name=>({name,command:[`${tmp}/${name}`],timeout_seconds:2,idempotency_required:true,required_output:`${name}-ok`,attribution:{owner_repo:'hugin',actor:'fixture-hugin'}}));
...
node - "$TMP/plan.json" "$TMP/mismatched-plan.json" <<'NODE'
const fs=require('fs');const [a,b]=process.argv.slice(2);const x=require(a);x.plan_id='plan-evil';fs.writeFileSync(b,JSON.stringify(x));
NODE
